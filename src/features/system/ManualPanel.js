export default class ManualPanel {
        ...
        return [
                ...
                left: `
                    ...
                    <div class="flex-1 flex flex-col justify-center mt-4">
                        ...
                        <div class="sketch p-2 bg-slate-50 w-full mx-auto">
                            <img src="src/assets/manual/image-workspace-demo.png" alt="圖片工作區" class="w-full rounded block">
                        </div>
                    </div>
                `,
                right: `
                    ...
                    <div class="flex-1 flex flex-col justify-center mt-4">
                        ...
                        <div class="sketch p-2 bg-slate-50 w-full mx-auto">
                            <img src="src/assets/manual/pdf-workspace-demo.png" alt="PDF 工作區" class="w-full rounded block">
                        </div>
                    </div>
                `
            },
                                ...
                                <div class="space-y-6">
                                    <div>
                                        ...
                                        <div class="sketch p-1 bg-slate-50">
                                            <img src="src/assets/manual/select-drag-demo.gif" class="w-full rounded block">
                                        </div>
                                    </div>
                                    <div>
                                        <p class="text-sm text-slate-600 mb-2"><strong>操作 2：</strong>拖曳畫出紅色選取框</p>
                                        <div class="sketch p-1 bg-slate-50">
                                            <img src="src/assets/manual/select-box-demo.gif" class="w-full rounded block">
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ${this.generateToolBlock('fas fa-hand-paper', '平移畫布', '按住滑鼠左鍵拖曳即可平移。', 'src/assets/manual/pan-demo.gif')}
                        ${this.generateToolBlock('fas fa-shapes', '加入形狀', '在畫布上產生矩形，並可透過左側「屬性」面板調整顏色與邊框樣式。', 'src/assets/manual/shape-demo.gif')}
                    </div>
                `,
                right: `
                    <h3 class="text-2xl font-bold text-indigo-700 border-b-2 border-indigo-200 pb-2 mb-6">基礎工具介紹 (2/2)</h3>
                    <div class="space-y-8">
                        ${this.generateToolBlock('fas fa-paint-brush', '塗鴉筆刷', '自由在畫布上繪製塗鴉。', 'src/assets/manual/brush-demo.gif')}
                        ${this.generateToolBlock('fas fa-search-plus', '放大畫布', '點擊畫布以放大視野。', 'src/assets/manual/zoom-in-demo.gif')}
                        ${this.generateToolBlock('fas fa-search-minus', '縮小畫布', '點擊畫布以縮小視野。', 'src/assets/manual/zoom-out-demo.gif')}
                    </div>
                `
            },
            // Page 4: Smart Tools
            {
                ...
                left: `
                    ...
                    <div class="space-y-8">
                        ${this.generateToolBlock('fas fa-magic', '一鍵去背', '全自動辨識主要主體並移除背景。<br><span class="text-orange-500 font-bold">※ 等待模型處理需要一點時間</span>', 'src/assets/manual/auto-rmbg-demo.gif')}
                        ${this.generateToolBlock('fas fa-eraser', '塗抹修補', '塗抹想要修飾的瑕疵，AI 會自動填補。<br><span class="text-orange-500 font-bold">※ 等待模型處理需要一點時間</span>', 'src/assets/manual/brush-inpaint-demo.gif')}
                        ${this.generateToolBlock('fas fa-cut', '選框去背', '框選想要去背的範圍，僅針對該範圍內的主體去背。<br><span class="text-orange-500 font-bold">※ 等待模型處理需要一點時間</span>', 'src/assets/manual/area-rmbg-demo.gif')}
                    </div>
                `,
                right: `
                    <h3 class="text-2xl font-bold text-indigo-700 border-b-2 border-indigo-200 pb-2 mb-6">智慧去背與修補 (2/2)</h3>
                    <div class="space-y-8">
                        ${this.generateToolBlock('fas fa-band-aid', '智慧修補', '框選不要的物件，讓它一秒消失。', 'src/assets/manual/inpaint-demo.gif')}
                        ${this.generateToolBlock('fas fa-font', '智慧辨識 (OCR)', '框選圖片文字，轉換為純文字。', 'src/assets/manual/ocr-demo.gif')}
                        ${this.generateToolBlock('fas fa-square', '純色覆蓋', '吸取顏色並覆蓋圖片上的資訊。', 'src/assets/manual/solid-fill-demo.gif')}
                    </div>
                `
            },
            // Page 5: Add Objects
            {
                ...
                left: `
                    ...
                    <div class="space-y-8">
                        ${this.generateToolBlock('fas fa-text-width', '新增文字', '點擊新增文字方塊，支援多種中英文字體。', 'src/assets/manual/text-demo.gif')}
                        ${this.generateToolBlock('fas fa-table', '建立表格', '繪製可調整欄列寬度與背景色的表格。', 'src/assets/manual/table-demo.gif')}
                    </div>
                `,
                right: `
                    <h3 class="text-2xl font-bold text-indigo-700 border-b-2 border-indigo-200 pb-2 mb-6">加入新物件 (2/2)</h3>
                    <div class="space-y-8">
                        ${this.generateToolBlock('fas fa-qrcode', 'QR 條碼', '輸入網址，一鍵產生向量 QR 條碼。', 'src/assets/manual/qrcode-demo.gif')}
                        ${this.generateToolBlock('fas fa-image', '外部圖片', '上傳電腦中的圖片 (JPG/PNG)。', 'src/assets/manual/image-demo.gif')}
                    </div>
                `
            },
            // Page 6: Misc Settings
            {
                ...
                left: `
                    ...
                    <div class="sketch p-2 bg-slate-50 w-full mx-auto mb-6">
                        <img src="src/assets/manual/ratio-switch-demo.gif" alt="比例切換" class="w-full rounded block">
                    </div>
                ...
                right: `
                    ...
                    <div class="space-y-8">
                        ${this.generateToolBlock('fas fa-expand', '全螢幕模式', '隱藏所有干擾，專注於畫布編輯。', 'src/assets/manual/fullscreen-demo.mp4')}
                        ${this.generateToolBlock('fas fa-layer-group', '圖層快速預覽', '快速查看與調整物件的前後圖層順序。', 'src/assets/manual/layers-demo.gif')}
                        ${this.generateToolBlock('fas fa-comment-dots', '留言評論', '與團隊成員在畫布上進行協作討論。', 'src/assets/manual/comments-demo.gif')}
                    </div>
                `
            },
    ...
    generateToolBlock(icon, title, desc, imgSrc) {
        const finalSrc = (imgSrc.startsWith('src/') || imgSrc.startsWith('http')) 
            ? imgSrc 
